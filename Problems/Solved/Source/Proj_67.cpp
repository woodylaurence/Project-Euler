 ...
 *      Author: Laurence
 *
 *      Problem: Find maximal sum by travelling from top of trianlge to base
 *
 *      NB. This is exactly the same algorithm as Problem 18, except NUM_LINES is 100 now and file is triangle_100.txt
 */
...
class Prob67
{
	...
	void readInTriangle(vector<int> *tmpTriangle)
	{
		...
		input = new ifstream("triangle_100.txt");
		if (input->is_open())
		{
			for (int i = 0; i < NUM_LINES; i++)
			{
